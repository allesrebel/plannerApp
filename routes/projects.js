...
router.get('/', function (req, res, next) {
    ...
    if (req.query.name) {
        // perform partial search based on name
        ...
        // NOTE: because we're simulating/mocking a DB, we need to
        // filter out any nulls due to deletes/upates
        const filtered = appDatabase.projects.filter(
...
router.get('/:id', function (req, res, next) {
    // emulate check if database has the project via search
    ...
    if (searchResults.length !== 1) {
        ...
        const updatedView = searchResults.at(0);
        const taskCount = updatedView.task_ids.length;
        const userCount = updatedView.user_ids.length;
        updatedView.userCount = userCount;
        updatedView.taskCount = taskCount;
...
router.put('/:id', function (req, res, next) {
    ...
    // to invlude a different set of fields instead
    delete appDatabase
        .projects[appDatabase.projects.indexOf(idSearchResults[0])];
